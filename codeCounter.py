...
class code_counter_app(object):
    ...
    #my_pattern = re.compile(r'[a-zA-Z1-9-_]+.(py|c|java|php|cpp|css|html|xml|htm|js|cs|h|asm|sh|ruby|perl)$')
    my_pattern = re.compile(r'[a-zA-Z1-9-_]+.(py)$')
    ...
    def count_code_of_file(self, file_name):
        ...
        commentline, blankline, codeline = 0, 0, 0
        lineno, iscomment = 0, False
        for eachLine in f:
            lineno += 1
            if eachLine.find("'''") == 0 or eachLine.find('"""') == 0:
